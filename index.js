...
app.post('/searchventilatorbystatus',middleware.checkToken, (req, res) => {
    ...
    var data = db.collection("Ventilator_Details").find({ "status": status }).toArray().then(result => res.json(result));
});
...
app.post('/searchventilatorbyid',middleware.checkToken, (req, res) => {
    ...
    var data = db.collection("Ventilator_Details").find({ "hid": hid}).toArray().then(result => res.json(result));
});
...
app.post('/searchventilatorbyhospitalname',middleware.checkToken, (req, res) => {
    ...
    var data = db.collection("Ventilator_Details").find({ "name": new RegExp(name,'i')}).toArray().then(result => res.json(result));
});
...
app.put('/deleteventilatorbyid',middleware.checkToken,(req,res)=>{
    ...
    db.collection("Ventilator_Details").deleteOne(ventid,function (err,result){
